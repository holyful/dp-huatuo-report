...
Util = {
	...
	cacheRequest: function(name, memcached, memcacheKey, options, gapTimestamp, res, req){
		...
		this.REQ_OPTIONS[memcacheKey] = options;
		memcached.get(memcacheKey, function (err, data) {
			...
			//console.log(memcacheKey);
			//console.log(self.REQ_OPTIONS);
			if(!data || DEBUG){
				res.setHeader("Cache","MISS");
				if(!MOCK){

					console.log(self.REQ_OPTIONS[selfMem.ke])
	...
	cacheMutiRequest: function(name, memcached, memcacheKey, options, gapTimestamp, res, req, handler){
		var deferred = Q.defer();
		var DEBUG = !!(req.query.debug * 1) || false;
		var MOCK = !!(req.query.mock * 1) || false;
		memcached.get(memcacheKey, function (err, data) {
			...
			if(!data || DEBUG){
				...
				if(!MOCK){
					...
					res.send(handler(require(MOCK_URL[name])));
				}
				return;
